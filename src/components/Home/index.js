...
class Home extends Component {
  ...
  onClickThumnailUrl = thumbnail => {
    const {imgUrl} = this.state
    const imageValue = imagesList.filter(
      eachValue => eachValue.thumbnailUrl === thumbnail,
    )
  ...
  render() {
    const {time, isTrue, imgUrl, category, score} = this.state
    const filterResults = imagesList.filter(
                ...
                {tabsList.map(eachItem => (
                  <li key={eachItem.tabId}>
                    <button
                      type="button"
                      className={`tab-btn ${
                        eachItem.tabId === category ? 'highlight-text' : ''
                      }`}
                ...
                {filterResults.map(eachItem => (
                  <li key={eachItem.id}>
                    <button
                      type="button"
                      onClick={() =>
                        this.onClickThumnailUrl(eachItem.thumbnailUrl)
                      }
                      className="thumnail-btn"
                    >
                      <img
